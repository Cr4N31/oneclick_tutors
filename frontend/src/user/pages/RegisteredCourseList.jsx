...
function RegisteredCourseList({ user, onUserUpdate }) {
  ...
  const handleSave = () => {
    ...
    onUserUpdate?.({
      ...
      raw_courses: cleanCourses.map((c) => c.code), // keep raw_courses in sync
    });
    setEditing(false);
  };
  ...
  const handleAdd = () => {
    ...
    setCourses([
      ...courses,
      { code: `COURSE ${courses.length + 1}`, name: "New course" },
    ]);
  };
    ...
    <main>
      ...
      <div className="mt-8 pt-6 border-t border-[#3D0A4F]/6">
        <p className="text-xs text-[#3D0A4F]/40">
          You can register between 9 and 12 courses per session. Click edit
          courses to update your list.
        </p>
      </div>
    </main>
  );
